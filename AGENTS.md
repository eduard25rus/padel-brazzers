...
- Homepage data rules: "Ближайшие турниры" contains only tournaments whose start time is still in the future; never backfill it with archive entries or demo fallbacks. When empty, show an explicit empty-state. The homepage leader module always has three honest data slots: best PRO player, best LITE player, and best forecaster. The hero CTAs are "Сделать прогноз" and "Смотреть лидеров" and must navigate to those sections.
